...
/**
 * Created by Yoseph Alabdulwahab on 3/29/2016. This is a lookup table class. It
 * can be used to simulate functions based on known data points
 */
public class LinearLookupTable {
     ...
     *
     * @param x
     *            the array of x values for this table. Must be in order.
     * @param y
     *            the array of y values for this table. Must be in order.
     */
    public LinearLookupTable(double[] x, double[] y) {
        if (x.length != y.length || x.length == 0)
            throw new RuntimeException(
                    "Could not create a lookup table with different number of x and y values.");
        this.xArr = x;
        this.yArr = y;
    }
    ...
    /**
     * @param x
     *            the x value that is being searched for.
    ...
    public double getClosestValue(double x) {
        ...
        if (b <= 0 || b >= xArr.length - 1)
            return yArr[b];
        int a = b - 1;
        if (Math.abs(Math.abs(xArr[a]) - Math.abs(x)) > Math
                .abs(Math.abs(xArr[b]) - Math.abs(x)))
            return yArr[a];
        return yArr[b];
    }

    /**
     * Returns the location of that element, or the location of the next biggest
     * element.
     *
     * @param a
     *            the specified array.
     * @param key
     *            the value desired to search for.
