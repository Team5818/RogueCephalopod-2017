 ...
 * @author jproney
 */
public class DriveTrajectoryVision extends Command {
    ...
    public DriveTrajectoryVision(double dist, double head, double startVel, double endVel, Direction dir, boolean stop) {
        distance = dist;
    ...
    protected void initialize() {
        loopCount = 0;
        double curHeading = -Math.toRadians(vision.getCurrentAngle());
        if(Double.isNaN(curHeading)){
            curHeading = goalHeading;
        }
    ...
    protected void execute() {
        ...
        double observedHeading = -Math.toRadians(vision.getCurrentAngle());

        
        double angleDiffRads = MathUtil.wrapAngleRad(goalHeading - observedHeading);
        if(Double.isNaN(observedHeading)){
            angleDiffRads = 0;
        }
